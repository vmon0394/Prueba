 ...
 */
$aColumns = array('idPrestamo','nombre','apellido','nombreMaterial','fechaPrestamo','fechaDevolucion','isdelPrestamo');
$sJoin = "LEFT JOIN tbl_usuario_laboratorio ON tbl_usuario_laboratorio.documento = tbl_prestamo_devolucion.documento LEFT JOIN tbl_inventario ON tbl_inventario.codigo = tbl_prestamo_devolucion.codigo";


/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "idPrestamo";

/* DB table to use */
$sTable = "tbl_prestamo_devolucion";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idPrestamo") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "isdelPrestamo") {
        ...
        $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='enable' target='_blank'><img src='../img/chulo.png' class='img-responsive img-rounded' width='35%' height='35%' title='Articulo Entregado'/></a></center>";
