 ...
 */
$aColumns = array('idMaterial', 'nombreMaterial', 'codigo', 'estado', 'isdelMaterial');
$sJoin = " INNER JOIN tbl_zonas2 ON tbl_zonas2.idZona = tbl_inventario.idZona ";

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "idMaterial";

/* DB table to use */
$sTable = "tbl_inventario";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idMaterial") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "isdelMaterial") {
