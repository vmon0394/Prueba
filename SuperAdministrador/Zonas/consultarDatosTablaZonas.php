 ...
 */
$aColumns = array('idZona', 'nombreZona', 'municipio', 'isdelZona');
$sJoin = " INNER JOIN tbl_municipios ON tbl_municipios.idMunicipio = tbl_zonas.idMunicipio";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idZona") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "isdelZona") {
    ...
    $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='consult' target='_blank'><img src='../img/consultar.png' class='img-responsive img-rounded' width='30%' height='30%' title='Consultar Zona'/></a></center>";
    if ($estado == "1") {
        $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='delete' target='_blank'><img src='../img/dele.png' class='img-responsive img-rounded' width='40%' height='40%' title='Eliminar Zona'/></a></center>";
    }
//    else if ($estado == "0") {
