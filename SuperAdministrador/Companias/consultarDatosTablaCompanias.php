 ...
 */
$aColumns = array('id_compania', 'nombre_compania', 'isdelCompania');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "id_compania";

/* DB table to use */
$sTable = "tbl_compania_relacionadas";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "id_compania") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "isdelCompania") {
    ...
    $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='consult' target='_blank'><img src='../img/consultar.png' class='img-responsive img-rounded' width='30%' height='30%' title='Consultar Compañia'/></a></center>";
    if ($estado == "1") {
        $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='delete' target='_blank'><img src='../img/dele.png' class='img-responsive img-rounded' width='35%' height='35%' title='Eliminar Compañia'/></a></center>";
    } 
