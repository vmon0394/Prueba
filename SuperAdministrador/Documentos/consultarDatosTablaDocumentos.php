 ...
 */
$aColumns = array('id_documento', 'nombre_documento', 'url_documento');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "id_documento";

/* DB table to use */
$sTable = "tbl_documentos_fundacion";
...
$sWhere = " WHERE isdel_documento = '1' ";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "id_documento") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "url_documento") {
            $url_documento = $aRow[$aColumns[$i]];
        } else {
            $row[] = $aRow[$aColumns[$i]];
        }
    }
    //Lineas de codigo para cambiar los espacios y los # que no se pueden en la URL
    $url_documento = str_replace(" ", '%20', $url_documento);
    $url_documento = str_replace("#", '%23', $url_documento);

    $row[] = "<center><a href='../$url_documento' title='' data-toggle='modal' class='see' target='_blank'><img src='../img/download.png' class='img-responsive img-rounded' width='50%' height='50%' title='Descargar Documento'/></a></center>";
    $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='delete' target='_blank'><img src='../img/dele.png' class='img-responsive img-rounded' width='35%' height='35%' title='Eliminar Documento'/></a></center>";
