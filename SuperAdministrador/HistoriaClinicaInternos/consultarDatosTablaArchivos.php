...
$idHistoria = isset($_GET['consecutivo']) ? $_GET['consecutivo'] : '0';
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Variables de las colummnas
 */
$aColumns = array('idArchivoExterno', 'fechaExterno', 'observacionExterno', 'urlExterno');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "idArchivoExterno";

/* DB table to use */
$sTable = "tbl_archivos_historia_externo";
...
$sWhere = " WHERE idHistoriaExterno = '$idHistoria' ";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idArchivoExterno") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "urlExterno") {
            $url = $aRow[$aColumns[$i]];
        } else {
            $row[] = $aRow[$aColumns[$i]];
        }
    ...
    $row[] = "<center><a href='../$url' title='' data-toggle='modal' class='see' target='_blank'><img src='../img/ojo.png' class='img-responsive img-rounded' width='40%' height='40%' title='Ver Archivo'/></a></center>";
    $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='delete' target='_blank'><img src='../img/dele.png' class='img-responsive img-rounded' width='35%' height='35%' title='Eliminar Archivo'/></a></center>";
