 ...
 */
$aColumns = array('idHistoParticipante', 'nombreSemillero', 'anoHistoParticipante');
$sJoin = " INNER JOIN tbl_semilleros ON tbl_semilleros.idSemillero = tbl_historial_participante_semilleros.idSemillero ";

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "idHistoParticipante";

/* DB table to use */
$sTable = "tbl_historial_participante_semilleros";
...
$sWhere = " WHERE idParticipante = '$idFicha' ";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idHistoParticipante") {
            $ID = $aRow[$aColumns[$i]];
