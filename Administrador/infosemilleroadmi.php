    ...
    <body>
        <div class="container-fluid content-wrapper">
            ...
            <div class="row-fluid">                
                ...
                <hr>
                <?php
                $tallerforma   =   psicosocialcontar::talleresformativossemi(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Talleres Formativos: ".$row["tformativos"]."</h4>";
                    }
                ?>
                <hr>
                <?php
                ...
                ?>
                 <hr>
                  <?php
                $tallerforma   =   psicosocialcontar::Actividades_espe(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Actividades Especiales: ".$row["actiespe"]."</h4>";
                    }
                ?>
                 <hr>
                 <?php
                $tallerforma   =   psicosocialcontar::salidaspedagogicassemi(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Salidas Pedagógicas: ".$row["salidaspe"]."</h4>";
                    }
                ?>
                 <hr>
                <?php
                $tallerforma   =   psicosocialcontar::vacacionesrecreativas(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Vacaciones recreativas: ".$row["vacaciones"]."</h4>";
                    }
                ?>
                 <hr>
                <?php
                ...
                ?>
                 <hr>
                <?php
                $tallerforma   =   psicosocialcontar::Campamento(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Salidas a Campamento: ".$row["Campamento"]."</h4>";
                    }
                ?>
                 <hr>
                <?php
                $tallerforma   =   psicosocialcontar::Muestra(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Muestra: ".$row["Muestra"]."</h4>";
                    }
                ?>
                 <hr>
                <?php
                $tallerforma   =   psicosocialcontar::Cierre(base64_decode($_REQUEST["ui"]));
                    foreach ($tallerforma as $row ) {
                        echo" <h4>Cierre: ".$row["Cierre"]."</h4>";
                    }
                ?>
                 <hr>
                <?php
