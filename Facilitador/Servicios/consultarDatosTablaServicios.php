 ...
 */
$aColumns = array('idServicio', 'nombreServicio', 'isdelServicio');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "idServicio";

/* DB table to use */
$sTable = "tbl_servicios";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idServicio") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "isdelServicio") {
    ...
    $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='consult' target='_blank'><img src='../img/consultar.png' class='img-responsive img-rounded' width='30%' height='30%' title='Consultar Servicio'/></a></center>";
    if ($estado == "1") {
        $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='delete' target='_blank'><img src='../img/dele.png' class='img-responsive img-rounded' width='35%' height='35%' title='Eliminar Servicio'/></a></center>";
    } 
//    else if ($estado == "0") {
