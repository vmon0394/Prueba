 ...
 */
$aColumns = array('idSalida', 'salida', 'fechaSalida', 'numParticipantes','descripcion', 'isdelSalida');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "idSalida";
...
$sWhere = " WHERE isdelSalida = '1' AND fechaSalida BETWEEN '2017-01-01' AND '2017-12-31'";
...
//    $row[] = $conta; //Nº
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        /* General output */
        if ($aColumns[$i] == "idSalida") {
            $ID = $aRow[$aColumns[$i]];
            $row[] = "<center>" . $conta . "</center>";
        } else if ($aColumns[$i] == "isdelSalida") {
    ...
    $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='consult' target='_blank'><img src='../img/consultar.png' class='img-responsive img-rounded' width='30%' height='30%' title='Consultar Salida'/></a></center>";
    if ($estado == "1") {
        $row[] = "<center><a href='#' title='$ID' data-toggle='modal' class='delete' target='_blank'><img src='../img/dele.png' class='img-responsive img-rounded' width='35%' height='35%' title='Eliminar Salida'/></a></center>";
    } 
//    else if ($estado == "0") {
