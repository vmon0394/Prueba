                ...
                <section id="semilleros" style='position:relative;display:none'>  
                       <?php
                            ...
                            if (!$conexion) {
                                ...
                                $sql = "SELECT * FROM tbl_semilleros WHERE idProfesor = '".$usuario."' ORDER BY nombreSemillero";
                                $rs = $conexion->query($sql);
                                    while ($numero = $rs->fetch_array()){
                                    echo" 
                                        <a href='infosemillerofaci.php?ui=".base64_encode($numero["idSemillero"])."'><button type='button' class='btn btn-primary' data-dismiss='modal'>".$numero["nombreSemillero"]."</button></a><br><br>";
                                     }
                                     $link->desconectar();
                                } 
                        ?>
