...
//                    echo $anosDePermanencia . " - " . $anosDeshabilitados;
    ...
    function validarAsisteciaTalleresAdmin() {
        ...
        if (!$conexion) {
            ...
            $sql = "SELECT f.idFicha, s.idSemillero, s.nombreSemillero, f.documento, f.nombres, f.apellidos FROM tbl_ficha_sociofamiliar f INNER JOIN tbl_semilleros s ON f.idSemillero = s.idSemillero "
                    . "WHERE f.isdel = '1'";
            $rs = $conexion->query($sql);
            ...
            if ($rs->num_rows > 0) {
                ...
                while ($aRow = $rs->fetch_array()) {
                    ...
                    $sqlT = "SELECT cadenaIdNinosTaller, asistenciaTaller FROM `tbl_talleres` WHERE idSemillero = '$idSemillero' ";
        ...
        if (!$conexion) {
            ...
            if ($rs->num_rows > 0) {
                ...
                while ($aRow = $rs->fetch_array()) {
                    ...
                    $sqlT = "SELECT cadenaIdNinosTaller, asistenciaTaller FROM `tbl_talleres` WHERE idSemillero = '$idSemillero' AND tipoTaller <> 'Taller Psicosocial'";
                    $rsT = $conexion->query($sqlT);
    ...
    function validarAsisteciaTalleresPsico() {
     ...
     function contarFichas() {
        ...
        if (!$conexion) {
            ...
            $sql = "SELECT count(*) AS contarfichas FROM `tbl_ficha_sociofamiliar`";
            $rs = $conexion->query($sql);
            if ($numero = $rs->fetch_assoc()){
                    $this->result = "<h3>Total de Fichas Participantes Registradas: ".$numero['contarfichas']."</h3>";
