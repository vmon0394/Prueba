...
class libEvidenciasActividades {
    ...
    //archivo físicamente.
    function cargarEvidenciaActividad() {
                ...
                if (!file_exists($output_dir)) {
                    ...
                    if (!is_array($_FILES["file"]["name"])) {
                        ...
                        if (copy($_FILES["file"]["tmp_name"], $output_dir)) {
                            ...
                            $sql2 = "SELECT idServicio, fecha, nombreActividad FROM `tbl_actividades_laboratorio` WHERE idActividad = '$this->idActividad' ";
                            ...
                            $sql = "INSERT INTO `tbl_evidencias_actividades`(`idActividad`, `fecha`, `nombreActividad`, `idServicio`, `nombreEvidencia`, `url_evidencia`, `anoEvidencia`) "
                                    . "VALUES ('$this->idActividad', '$fecha', '$nombreActividad', '$idServicio', '$this->fileName', '$url', '$this->anoRegistro');";
    ...
    //dicho archivo.
    function eliminarEvidenciaActividad() {
        ...
        if (!$conexion) {
            ...
            $sql1 = "SELECT url_evidencia FROM `tbl_evidencias_actividades` WHERE idEvidencia = '$this->idEvidencia'";
            ...
            if ($rs1->num_rows > 0) {
                ...
                $sql = "DELETE FROM `tbl_evidencias_actividades` WHERE idEvidencia = '$this->idEvidencia'";
                $rs = $conexion->query($sql);
