$(document).ready(function () {
    ...
    $("#p4").click(function (e) {
        ...
        if ($('#nombresPadre').val() === "" || ($('#telefonoPadre').val() === "" && $('#celularPadre').val() === "") || $('#tipologia').val() == 0 || $('#numeroMiembros').val() === ""
                || $('#etnia').val() == 0) {
            var modal = $('#exampleconfirmacion');
            modal.find('.modal-title').text("Faltan Datos");
    ...
    $("#enviarFotoPerfil").click(function (e) {

        var file_data = $("#formUploadFotoPerfil").find('[name="archivoFotoPerfil"]').prop('files')[0];

        var form_data = new FormData();
        form_data.append('file', file_data);
        form_data.append('lider', $("#idFichaNinos").val());
        form_data.append('tipoArchivo', "FotoPerfil");
        form_data.append('folder', "FotosPerfiles");
        ...
        if (typeof file_data === "undefined") {
            ...
            if (file_data.size > 2000000 || (extD !== "pdf" && extD !== "PDF" && extD !== "png" && extD !== "PNG" && extD !== "jpg" && extD !== "JPG" && extD !== "jpeg" && extD !== "JPEG") || extD.size > 1000000) {
                ...
                $("#LoadingImage").show();
                $.ajax({
                    url: '../Controller/ctrlDocumentos.php?opcion=3', // point to server-side PHP script 
                    ...
                    success: function (result) {
                        ...
                        if (data.res === 'fail') {
                            ...
                            $("#imagenPerfil").html(data.row);

                            $("#formUploadFotoPerfil").each(function () {
                                this.reset();
                            });
...
function validarDatosPaso3Ninos() {
    ...
    if ($('#nombresPadre').val() === "") {
    ...
    } else if ($('#telefonoPadre').val() === "" && $('#celularPadre').val() === "") {
        ...
        var modal = $('#exampleconfirmacion');
        modal.find('.modal-title').text("Faltan Datos");
        modal.find('#modal-message').html("<div style='color: red'> Ingrese el teléfono y/o celular de la Mamá o Papá del niño. </div>");
        $('#exampleconfirmacion').modal('show');
