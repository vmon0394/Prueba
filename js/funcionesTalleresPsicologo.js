$(document).ready(function () {
    ...
    $("#p2T").click(function (e) {
        ...
        if ($("#semilleroTaller").val() == 0 || $("#tipoTaller").val() == 0 || $("#nombreTaller").val() === "" || $("#fechaTaller").val() === ""
                || $("#idHabilidad").val() == 0 || $("#valorTaller").val() == 0 || $("#tecnicaTaller").val() == 0 || $("#tipoActividad").val() === ""
    ...
    $('#saveTalleres').click(function () {
        ...
        if (Paso1Taller == false) {
            ...
            $.ajax({
                type: "POST",
                url: "../Controller/ctrlTalleresPsico.php?opcion=1",
    ...
    $('#updateTalleres').click(function () {
        ...
        if (Paso1Taller == false) {
            ...
            $.ajax({
                type: "POST",
                url: "../Controller/ctrlTalleresPsico.php?opcion=3",
                data: $("#frmTaller").serialize(),
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log(textStatus + "\n" + errorThrown);
                },
...
function tablaTaller() {
    ...
    $('#tblRolTalleres').dataTable({
        ...
        "bPaginate": true,
        "iDisplayLength": 10,
        "sAjaxSource": "TalleresPsico/consultarDatosTablaTalleres.php",
        "order": [[3, "asc"]],
        "aoColumnDefs": [{
                "aTargets": [2],
                "sType": "html"
...
function tomaAsistenciaPsico(taller) {
...
function validarPaso1Taller() {
    ...
    if ($('#semilleroTaller').val() == 0) {
        ...
        var modal = $('#exampleconfirmacion');
        modal.find('.modal-title').text("Faltan Datos");
        modal.find('#modal-message').html("<div style='color: red'> Ingrese el tiempo del taller. </div>");
        $('#exampleconfirmacion').modal('show');
