$(document).ready(function () {
    ...
    $("#enviarFoto").click(function () {
        ...
        if ($('#tallerEvidencia').val() == 0) {
            ...
            $("#tallerEvidencia").css({"border": "2px solid red", "box-shadow": "0 0 3px red"});

        } else {
            
            $("#tallerEvidencia").css({"border": "", "box-shadow": ""});

            var file_data = $("#formUploadEvidencia").find('[name="archivoEvidencia"]').prop('files')[0];

            var form_data = new FormData();
            form_data.append('file', file_data);
            form_data.append('idTaller', $("#tallerEvidencia").val());
            ...
            if (typeof file_data === "undefined") {
                ...
                if (extE.size > 1000000 || (extE !== "png" && extE !== "PNG" && extE !== "jpg" && extE !== "JPG" && extE !== "jpeg" && extE !== "JPEG")) {
                    ...
                    $.ajax({
                        url: '../Controller/ctrlEvidencias.php?opcion=1', // point to server-side PHP script 
                        ...
                        success: function (result) {
                            ...
                            if (data.res === 'fail') {
                                ...
                                tablaEvidencias($('#idSemillero').val());
                            }
    ...
    $('#tblRolEvidencias').on('click', 'a.delete', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlEvidencias.php?opcion=2",
            data: {idEvidencia: id},
...
function tablaEvidencias(idSemillero) {
    ...
    $('#tblRolEvidencias').dataTable({
        ...
        "bPaginate": true,
        "iDisplayLength": 10,
        "sAjaxSource": "Semilleros/consultarDatosTablaEvidencias.php?semillero=" + encodeURIComponent(idSemillero),
        "order": [[1, "desc"]],
        "aoColumnDefs": [{
                "aTargets": [2],
                "sType": "html"
