$(document).ready(function () {
    ...
    //funcion para el boton guardar 
        ...
        if ($('#nombre').val() === "") {
            ...
            modal.find('#modal-message').html("<div style='color: red'> Ingrese el nombre de la Actividad a registrar. </div>");
            ...
            var modal = $('#exampleconfirmacion');
            modal.find('.modal-title').text("Faltan Datos");
            modal.find('#modal-message').html(" <div style='color: red'> Ingrese la fecha de la Actividad. </div>");
            $('#exampleconfirmacion').modal('show');

            $("#fechaActividad").css({"border": "2px solid red", "box-shadow": "0 0 3px green"});
            
        } else if ($('#numParticipantes').val() === "") {

            $("#nombre").css({"border": "", "box-shadow": ""});
            $("#fechaActividad").css({"border": "", "box-shadow": ""});
            ...
            $.ajax({
                type: "POST",
                url: "../Controller/ctrlComunitarias.php?opcion=1",
                data: $("#frmComunitarias").serialize(),
    ...
    $('#tblRol').on('click', 'a.consult', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlComunitarias.php?opcion=2",
            data: {idComunitaria: id},
            error: function (jqXHR, textStatus, errorThrown) {
            ...
            success: function (result) {
                ...
                if (data.res === 'fail') {
                    ...
                    $("#fechaActividad").attr('disabled', 'disabled');
                    $("#descripcion").attr('disabled', 'disabled');
                    $("#numParticipantes").attr('disabled', 'disabled');
            

                    $("#idComunitaria").val(data.row.IdComunitaria);
                    $("#nombre").val(data.row.Nombre);
                    $("#fechaActividad").val(data.row.FechaActividad);
...
//                    $("#return").show();
    ...
    $('#modi').click(function () {
        ...
        $("#fechaActividad").removeAttr("disabled");
    ...
    $('#update').click(function () {
        ...
        if ($('#nombre').val() === "") {
            ...
            $.ajax({
                type: "POST",
                url: "../Controller/ctrlComunitarias.php?opcion=3",
                data: $("#frmComunitarias").serialize(),
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log(textStatus + "\n" + errorThrown);
                },
    ...
    $('#tblRol').on('click', 'a.delete', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlComunitarias.php?opcion=4",
            data: {idComunitaria: id},
            error: function (jqXHR, textStatus, errorThrown) {
                //*Ocurrió un eror en la petición ajax
                $("#LoadingImage").hide();
    ...
    $('#tblRol2').on('click', 'a.enable', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlComunitarias.php?opcion=5",
            data: {idComunitaria: id},
            error: function (jqXHR, textStatus, errorThrown) {
                //*Ocurrió un eror en la petición ajax
                $("#LoadingImage").hide();
...
function tabla() {
    ...
    $('#tblRol').dataTable({
        ...
        "bPaginate": true,
        "iDisplayLength": 10,
        "sAjaxSource": "ProyectosCC/consultarDatosTablaComunitarias.php",
        "order": [[1, "asc"]],
        "aoColumnDefs": [{
                "aTargets": [2],
                "sType": "html"
...
function tabla2() {
    ...
    $('#tblRol2').dataTable({
        ...
        "bPaginate": true,
        "iDisplayLength": 10,
        "sAjaxSource": "ProyectosCC/consultarDatosTablaComunitariasEliminadas.php",
        "order": [[1, "asc"]],
        "aoColumnDefs": [{
                "aTargets": [2],
                "sType": "html"
