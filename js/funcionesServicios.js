...
$(document).ready(function () {
    ...
    $('#enviarServicio').click(function () {
        ...
        var file_data = $("#frmUploadServicios").find('[name="archivoServicio"]').prop('files')[0];

        var form_data = new FormData();
        form_data.append('file', file_data);
        form_data.append('nombreServicio', $("#nombreServicio").val());
        ...
        if (typeof file_data === "undefined") {
            ...
            if (file_data.size > 2000000 || (extD !== "pdf" && extD !== "PDF")) {
                ...
                modal.find('#modal-message').html("<div style='color: red'> Ingrese el nombre del servicio. </div>");
                $('#exampleconfirmacion').modal('show');

                $("#nombreServicio").css({"border": "2px solid red", "box-shadow": "0 0 3px red"});
                
                } else if($('#facilitador').val()===0){

                    $("#nombreServicio").css({"border": "", "box-shadow": ""});
                    ...
                    $("#LoadingImage").show();
                    $.ajax({
                        url: '../Controller/ctrlServicios.php?opcion=1', // point to server-side PHP script 
                        ...
                        success: function (result) {
                            ...
                            if (data.res === 'fail') {
                                ...
                                $("#frmUploadServicios").each(function () {
                                    this.reset();
                                });
    ...
    $('#tblRol').on('click', 'a.consult', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlServicios.php?opcion=2",
            data: {idServicio: id},
            ...
            success: function (result) {
                ...
                if (data.res === 'fail') {
                    ...
                    $("#nombreServicio").val(data.row.NombreServicio);
                    $("#facilitador").val(data.row.IdFacilitador);
        ...
        if (typeof file_data === "undefined") {
            ...
            if (extD !== "pdf" && extD !== "PDF") {
                ...
                $("#nombreServicio").css({"border": "2px solid red", "box-shadow": "0 0 3px red"});
                    ...
                    $("#LoadingImage").show();
                    $.ajax({
                        url: '../Controller/ctrlServicios.php?opcion=3', // point to server-side PHP script 
                        dataType: 'text', // what to expect back from the PHP script, if anything
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    $('#tblRol').on('click', 'a.delete', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlServicios.php?opcion=4",
            data: {idServicio: id},
            error: function (jqXHR, textStatus, errorThrown) {
                //*Ocurrió un eror en la petición ajax
                $("#LoadingImage").hide();
...
function tabla() {
    ...
    $('#tblRol').dataTable({
        ...
        "bPaginate": true,
        "iDisplayLength": 10,
        "sAjaxSource": "Laboratorio/consultarDatosTablaServicios.php",
        "order": [[1, "asc"]],
        "aoColumnDefs": [{
                "aTargets": [2],
                "sType": "html"
