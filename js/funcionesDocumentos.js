...
$(document).ready(function () {
    ...
    $('#enviarDocumento').click(function () {
        ...
        if ($('#documento').val() === "") {
            ...
            modal.find('#modal-message').html("<div style='color: red'> Ingrese el nombre del documento. </div>");
            ...
            var file_data = $("#frmUploadDocumento").find('[name="archivoDocumento"]').prop('files')[0];
            ...
            if (typeof file_data === "undefined") {
                ...
                $("#archivoDocumento").css({"border": "2px solid red", "box-shadow": "0 0 3px red"});
            } else {
                
                $("#archivoDocumento").css({"border": "", "box-shadow": ""});
                ...
                if (file_data.size > 2000000 || (extD !== "pdf" && extD !== "PDF")) {
                    ...
                    modal.find('#modal-message').html("<div style='color: red'> Verifique que el archivo seleccionado cumpla con el documento indicado pdf y/o el tamaño máximo.. </div>");
                    $('#exampleconfirmacion').modal('show');
                } else {

                    $("#LoadingImage").show();
                    $.ajax({
                        url: '../Controller/ctrlDocumentosf.php?opcion=1', // point to server-side PHP script 
                        ...
                        success: function (result) {
                            ...
                            if (data.res === 'fail') {
                                ...
                                $("#frmUploadDocumento").each(function () {
                                    this.reset();
                                });
    ...
    $('#tblRol').on('click', 'a.consult', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlDocumentosf.php?opcion=2",
            data: {idDocumento: id},
            ...
            success: function (result) {
                ...
                if (data.res === 'fail') {
                    ...
                    $("#idDocumento").val(data.row.Id_documento);
                    $("#documento").val(data.row.Nombre_documento);

                    $("#updateDocumento").show();
    ...
    $('#updateDocumento').click(function () {
        ...
        var form_data = new FormData();
        form_data.append('file', file_data);
        form_data.append('idDocumento', $("#idDocumento").val());
        form_data.append('documento', $("#documento").val());
        ...
        if (typeof file_data === "undefined") {
            ...
            if (file_data.size > 2000000 || (extD !== "pdf" && extD !== "PDF" )) {
                ...
                if ($('#documento').val() === "") {
                    ...
                    $("#LoadingImage").show();
                    $.ajax({
                        url: '../Controller/ctrlDocumentosf.php?opcion=3', // point to server-side PHP script 
                        dataType: 'text', // what to expect back from the PHP script, if anything
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    $('#tblRol').on('click', 'a.delete', function () {
        ...
        $.ajax({
            type: "POST",
            url: "../Controller/ctrlDocumentosf.php?opcion=4",
            data: {idDocumento: id},
            error: function (jqXHR, textStatus, errorThrown) {
                //*Ocurrió un eror en la petición ajax
                $("#LoadingImage").hide();
...
function tabla() {
    ...
    $('#tblRol').dataTable({
        ...
        "bPaginate": true,
        "iDisplayLength": 10,
        "sAjaxSource": "Documentos/consultarDatosTablaDocumentos.php",
        "order": [[1, "asc"]],
        "aoColumnDefs": [{
                "aTargets": [2],
                "sType": "html"
